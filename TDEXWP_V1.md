...
Initiated by SevenLabs, TDEX is a community-oriented and Elements-based decentralised exchange built on the Liquid Network. Leveraging the combined security, usability, and speed of the Bitcoin-Liquid framework, TDEX envisions a fast, secure, and truly-decentralised trading experience in Liquid Assets (L-Assets).

The underlying TSWAP protocol implements atomic swaps and solves the liquidity problem through several &quot;zero-to-one&quot; incentives. TDEX also empowers liquidity providers by enabling them to choose their market-making strategy. On the other hand, users gain from competitive rates, faster settlements, lower slippage, and better security.

In the following sections, we discuss the background and foreground elements of the TDEX platform. In the process, we highlight how the platform could emerge as a backbone network for Trustless Finance, enabling professional traders and business owners to manage, transact, and invest within a privacy-prioritised environment.
...
In general, it must be borne in mind that assets traded on the TDEX network are essentially Liquid Assets. Thus, being a secondary market for L-Assets is among the primary use cases of the TDEX project, at least in the initial stages.
